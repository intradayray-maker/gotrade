...
import { getAlpacaConfigForUser } from "@/lib/alpacaClient";
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return NextResponse.json(
        { error: "Failed to fetch trades from Alpaca", details: text },
        { status: res.status }
      );
    }

    const activities = (await res.json()) as AlpacaActivity[];
    let fills = activities.filter((a) => a.activity_type === "FILL");

    if (side === "buy" || side === "sell") {
      fills = fills.filter((fill) => fill.side === side);
    }
    ...
    return NextResponse.json({
      data: fills.slice(startIndex, endIndex).map(normalizeTrade),
      page,
      totalPages,
    });
  } catch (err) {
    ...
    const message =
      err instanceof Error ? err.message : "Unexpected error fetching trades";
    return NextResponse.json(
      { error: message },
      { status: 500 }
    );
