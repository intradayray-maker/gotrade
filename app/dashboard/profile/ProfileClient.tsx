  ...
  useEffect(() => {
    async function loadAlias() {
      try {
        ...
        if (!res.ok) {
          throw new Error(`Alias fetch failed: ${res.status}`)
        }
...
setTimeout(() => setAliasSaved(false), 1500)
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(`Alias save failed: ${res.status}`)
      }
    } catch (err) {
...
-------------------------- */}
{passwordResetSent && (
  <GTCard className="border border-green-500/20">
...
{passwordResetError && (
  <GTCard className="border border-red-500/20">
...
>
  ...
  >
    <p className="text-white/90 text-[17px] font-semibold">
      {firstName || lastName ? `${firstName} ${lastName}`.trim() : "No display name set"}
    </p>
...
>
  ...
  >
    ...
    before:from-emerald-300/60
    before:to-emerald-700/60
    before:-z-10
...
before:p-[2px]
before:bg-gradient-to-br
before:from-red-300/60
before:to-red-700/60
before:-z-10
hover:bg-[rgb(110,45,45)]
transition
      "
      >
        CHANGE IT
  ...
  >
    ...
    >
      <div className="flex items-center gap-2">
        ...
        {billingStatus === "active" && (
          <div className="h-3.5 w-3.5 rounded-full bg-emerald-400 shadow-[0_0_10px_rgba(0,255,180,0.9)]"></div>
        )}
...
>
  ...
  {/* CELL 4 — FINAL CONFIRMATION TEXT */}
  <div
  className="
  border
  border-red-500/20
